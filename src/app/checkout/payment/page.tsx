        ...
        <CardContent>
          <Tabs defaultValue="card" className="w-full">
            <TabsList className="grid w-full grid-cols-3">
              ...
              <TabsTrigger value="klarna"><KlarnaIcon/>Klarna</TabsTrigger>
            </TabsList>
            <TabsContent value="card" className="mt-6">
                <div className="grid gap-4">
                    <div className="grid gap-2">
                        <Label htmlFor="card-name">Name on card</Label>
                        <Input id="card-name" placeholder="John Doe" required data-testid="card-name-input"/>
                    </div>
                     <div className="grid gap-2">
                        <Label htmlFor="card-number">Card number</Label>
                        <Input id="card-number" placeholder="**** **** **** 1234" required data-testid="card-number-input"/>
                    </div>
                    <div className="grid grid-cols-3 gap-4">
                        <div className="grid gap-2">
                             <Label htmlFor="expiry-date">Expiry date</Label>
                             <Input id="expiry-date" placeholder="MM/YY" required data-testid="expiry-date-input"/>
                        </div>
                        <div className="grid gap-2">
                             <Label htmlFor="cvc">CVC</Label>
                             <Input id="cvc" placeholder="123" required data-testid="cvc-input"/>
                        </div>
                    </div>
                </div>
            </TabsContent>
            <TabsContent value="paypal" className="mt-6 text-center">
                 ...
                 <Button className="w-full sm:w-auto" variant="outline"><PayPalIcon/>Continue with PayPal</Button>
            </TabsContent>
            <TabsContent value="klarna" className="mt-6 text-center">
                <p className="text-muted-foreground mb-4">Pay in installments with Klarna.</p>
                <Button className="w-full sm:w-auto" variant="outline"><KlarnaIcon/>Continue with Klarna</Button>
            </TabsContent>
          </Tabs>
          <div className="flex items-center justify-between mt-6">
            <Button variant="ghost" asChild><Link href="/checkout">Back to Shipping</Link></Button>
            <Button asChild size="lg">
              <Link href="/checkout/summary">Continue to Summary</Link>
            </Button>
          </div>
        </CardContent>
      </Card>
